...
class AuthTestCase(APITestCase):
    ...
    def test_basic_authentication(self):
        ...
        url_that_require_auth = '/api/v1/project/'
    ...
    def test_basic_authentication_error_cases(self):
        ...
        # POST an invalid user and get an error (401)
        resp = self.client.post(
            url,
            {
                "email": 'does.not.exists@netsach.org',
                "password": "email-does-not-exists",
            },
        )
        self.assertEqual(resp.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_authentication_failed_if_user_blocked(self):
        url = '/api/v1/auth/login/'
