...
class CRUDTestCaseWithoutScope(APITestCase):
    def setUp(self):
        ...
        self.role_1.users.add(self.user)

        # Create the permission linked to the RoleModel
        self.target_model_permission, _ = ConcretePermission.objects.get_or_create(
            model_name="RoleModel"
        )

        self.role_model_1 = RoleModel.objects.create(
            name="instance1", public=False
        )
    ...
    def test_create_role_wo_scope(self):
        ...
        self.assertEqual(RoleModel.objects.count(), 2)
    ...
    def test_retrieve_role_wo_scope(self):
        ...
        resp = self.client.get(
            '{}{}/'.format(url_role_model, str(self.role_model_1.uid)),
            HTTP_AUTHORIZATION='Token {}'.format(self.token),
        )
    ...
    def test_update_role_wo_scope(self):
        ...
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)
        self.role_model_1.refresh_from_db()
        self.assertEqual(self.role_model_1.name, 'instance1')
        ...
        self.role_model_1.refresh_from_db()
        self.assertEqual(self.role_model_1.name, 'New Instance Name')
    ...
    def test_delete_role_wo_scope(self):
        ...
        self.assertEqual(RoleModel.objects.count(), 0)
...
class CRUDTestCaseWithScope(APITestCase):
    def setUp(self):
        ...
        self.role_model_2 = RoleModel.objects.create(
            name="instance2", defaultdivider=self.divider_2
        )

        self.user.defaultdividers.add(self.divider_1)
    ...
    def test_delete_role_w_scope(self):
        ...
        self.assertEqual(RoleModel.objects.count(), 1)
